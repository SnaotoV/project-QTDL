                    ...
                    $allMon = $controlMon->getMonTheoUser($id);
                            if(!empty($allMon)):?>
                                <?php foreach($allMon as $mon): ?>
                                    <div><a href="<?=BASE_URL_PATH . 'allTest.php?makhoa='.$mon->makhoa.'&mamon=' . $mon->getIdMon() ?>" class='subject'><?=htmlspecialchars($mon->mamon)?> - <?=htmlspecialchars($mon->tenmon)?></a></div> 
                                    <?php endforeach?>
                                <?php else:?>
                                    <div>Giảng viên này chưa có môn dạy</div>
                                <?php endif?>
